...
//Scatterplot
var margin = {left: 100, top: 20, right: 20, bottom: 60},
	width = Math.min($("#chart").width(), 1000) - margin.left - margin.right,
	height = width/2;

var bubble = d3.select("#chart").append("svg")
			.attr("width", (width + margin.left + margin.right))
			.attr("height", (height + margin.top + margin.bottom));
			
var wrapper = bubble.append("g").attr("class", "chordWrapper")
...
var color = d3.scaleOrdinal()
            .range(["#EFB605", "#E01A25", "#991C71", "#2074A0", "#10A66E"])
            .domain(["Technology", "Global Issues", "Science", "Design", "Culture"]);
							 
//Set the new x axis range
var xScale = d3.scaleLinear()
	.range([0, width])
	.domain([2005,2018]); //I prefer this exact scale over the true range and then using "nice"
...
var yScale = d3.scaleLinear()
	.range([height,0])
	.domain(d3.extent(countries, function(d) { return d.Views; }))
	.nice();	
var yAxis = d3.axisLeft()
	.ticks(6)  //Set rough # of ticks
.tickFormat(function(d) { return parseInt(d / 1000000) + "M"; })
	.scale(yScale);	
//Append the y-axis
wrapper.append("g")
		.attr("class", "y axis")
...
var rScale = d3.scaleSqrt()
			.range([mobileScreen ? 3 : 4, mobileScreen ? 10 : 35])
			.domain(d3.extent(countries, function(d) { return d.Duration; }));
...
var voronoiCells = voronoi.polygons(countries);
...
clipWrapper.selectAll(".clip")
	...
	.enter().append("clipPath")
  	.attr("class", "clip")
  	.attr("id", function(d) { console.log(d); return "clip-" + d.data.CountryCode; })
  	.append("path")
...
var circlesOuter = circleClipGroup.selectAll(".circle-wrapper")
	.data(countries.sort(function(a,b) { return b.Duration > a.Duration; }))
	.enter().append("circle")
...
circleGroup.selectAll("countries")
	.data(countries.sort(function(a,b) { return b.Duration > a.Duration; })) //Sort so the biggest circles are below
	.enter().append("circle")
...
if (!mobileScreen) {
	//Legend			
	var	legendMargin = {left: 20, top: 20, right: 5, bottom: 10},
		legendWidth = 145,
		legendHeight = 270;
	...
	var rectSize = 19, //dimensions of the colored square
		rowHeight = 23, //height of a row in the legend
...
function selectLegend(opacity) {
	return function(d, i) {
		...
		wrapper.selectAll(".countries")
			.filter(function(d) { return d.Tag != chosen; })
			.transition()
...
function removeTooltip (d, i) {
	...
	var element = d3.selectAll(".countries."+d.CountryCode).transition().duration(200);
...
function showTooltip (d, i) {
	...
	$(el).popover({
		...
		html : true,
		content: function() { 
			return "<span style='font-size: 12px; text-align: center;'>" + "<span>" + "Title: " + "</span>" + d.Title + "\n" + "<span>" + "Speaker: " + "</span>" + d.Speaker + "\n" + "<span>" + "Duration: " + "</span>" + Math.round(d.Duration)  + " min"+"</span>"; }
	});
	$(el).popover('show');
